...
assert(verifyReplay(firstRun.replay), "generated replay hash must verify");
...
for (let seed = 0; seed < 100; seed += 1) {
  ...
  assert(verifyReplay(runA.replay), `replay verification failed at seed ${seed}`);
}
...
for (let firstIndex = 0; firstIndex < botNames.length; firstIndex += 1) {
  for (let secondIndex = firstIndex + 1; secondIndex < botNames.length; secondIndex += 1) {
    ...
    assert(verifyReplay(result.replay), `${botNames[firstIndex]} vs ${botNames[secondIndex]} replay did not verify`);
  }
}
...
console.log("- deterministic replay hash verified for 100 seeds");
